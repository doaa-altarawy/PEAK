...
# >>> ind is the index of the gene (row num) to calc its betas
callMEN <- function(ind, Xs, Y, clr.mat, 
                    nS,                   # max # of tf to take from CLR
                    nCv = 10,lambda = 0, 
                    sparseModels=T, 	
                    verbose = FALSE, 
                    plot.it = FALSE, 
                    ...
                    no.pr.val = no.pr.val,        # no prior val = 1
                    use_interactions = FALSE){
  
  # >>> the aplpha's for each beta (tf) ??
  ...
  # values for current gene with conditions 
  Y_ind <- Y[ind, ]
  #Xs <- t(scale(t(Xs[, Pi[ind, uniqConds]])))#
  # >>> matrix of (all genes X conditions)
  ...
  # >>>> selected tf X conditions
  # >>>> row names are the index of the tf
  ...
  cat("\nDone first call to CV elasticnet\n==============\n")
...
rescaleModels <- function(y, x, model.coefs, center = F, use.ev = F) {
  ...
  cat('\n')
  cat(model.coefs)
  # >>>>>>>>>>>>>>> ERROR here, NA is in model.coef
